$(function(){
	// 设置默认收货时间
	$('#damageDate').datebox('setValue',genTodayStr());
	
	// 设置时间组件可选的时间范围（不能小于当前日期）
	$('#damageDate').datebox('calendar').calendar({
	...
	// 设置单号
	$('#damageNumber').html(genOddNumbers('BS'));
 ...
 * 打开商品报损入库商品选择窗口
 */
function openDamageListGoodsDialog(){
	$('#dlg').dialog({
		title:'商品报损入库商品选择',
		closed:false,
		top:$(window).height()/6,
...
function saveDamageData(){
	$('#damageListGoodsStr').val(JSON.stringify($("#dg").datagrid("getData").rows));
	var damageNumber = $('#damageNumber').text();
	$('#fm').form('submit',{
		url:'/damageListGoods/save?damageNumber='+damageNumber,
		onSubmit:function(){
