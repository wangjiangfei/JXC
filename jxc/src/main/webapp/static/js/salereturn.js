$(function(){
	...
	$('#customerReturnDate').datebox('calendar').calendar({
	...
	//设置单号
	$('#customerReturnNumber').html(genOddNumbers('XT'));
 ...
 * 打开销售入库商品选择窗口
 */
function openCustomerReturnListGoodsDialog(){
	$('#dlg').dialog({
		title:'销售入库商品选择',
		closed:false,
		top:$(window).height()/6,
 ...
 * 保存销售清单
 */
function saveCustomerReturnData(){
	$('#customerReturnListGoodsStr').val(JSON.stringify($("#dg").datagrid("getData").rows));
	var customerReturnNumber = $('#customerReturnNumber').text();
	$('#fm').form('submit',{
		url:'/customerReturnListGoods/save?customerReturnNumber='+customerReturnNumber,
		onSubmit:function(){
			...
			if($('#customer').combobox('getData')==null || $('#customer').combobox('getData')=='' || 
					$('#customer').combobox('getValue')==null || $('#customer').combobox('getValue')==''){
				$.messager.alert({
					title:'系统提示',
					msg:'请选择客户',
					icon:'error', 
